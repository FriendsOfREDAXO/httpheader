...
if (!function_exists('aFM_maskArray')):
    ...
    {
        if (is_array($arr)):
            $arr = array_map(static function ($str) { return aFM_maskChar($str); }, $arr);
        endif;
...
if (!function_exists('aFM_blockTags')):
    ...
    {	// Entfernt bekannte Tags (PHP, JS, HTML)
        if ('' != $str):
            ...
            $str = strip_tags($str);
            $phps = ['/<\\?php/i', '/<\\?/i', '/<%/i', '/<script language="php">/i', "/<script language='php'>/i", '/\\?>/i', '/%>/i'];
            foreach ($phps as $key):
                $str = preg_replace($key, '', $str);
            endforeach;
            $js = ['/<script.*>/i', '/<\\/script>/i'];
            foreach ($js as $key):
                $str = preg_replace($key, '', $str);
            endforeach;
            $str = trim($str);
...
if (!function_exists('aFM_stripTags')):
    function aFM_stripTags($str, $allowed_tags = [])
    {	// Aufrufe: aFM_stripTags($str);			aFM_stripTags($str, array('h1','a'));
        $allowed_tags = array_map(strtolower, $allowed_tags);

        $rstr = preg_replace_callback('/<\/?([^>\s]+)[^>]*>/i', static function ($matches) use (&$allowed_tags) {
...
if (!function_exists('aFM_substr')):
    function aFM_substr($str, $limit = 100, $isUtf8 = true)
    {
        ...
        if ($retLength < $limit && $position < strlen($str)) {
            $ret .= substr($str, $position, $limit - $retLength);
